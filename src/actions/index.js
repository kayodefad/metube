...
export const addVideos = term => async dispatch => {
  const response = await axios.get(
    `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=50&key=AIzaSyCgTXlMdNUMIYEo7WDVWbCacITH-yPQcfs&q=${term}`
  )
...
export const playVideo = (id) => async dispatch => {
  const response = await axios.get(
    `https://www.googleapis.com/youtube/v3/search?part=snippet&maxResults=50&key=AIzaSyCgTXlMdNUMIYEo7WDVWbCacITH-yPQcfs&q=${id}`
  )
